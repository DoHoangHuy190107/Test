...
class GroupMonster(pygame.sprite.Group):   # Nhóm các spear
        ...
        for mons in self.sprites():
            if mons.HP < 1:
                ...
                if mons.dir > 0:
                    ...
                    mons.indexDL += 1
                if mons.indexDL == 4 or mons.indexDR == 4:
                    self.remove(mons)
                    numDef += 1
                else:
                    if mons.dir > 0:
                        mons.image = pygame.transform.scale(mons.arc_diedR[mons.indexDR], (sizeW, sizeH))
                    else:
                        mons.image = pygame.transform.scale(mons.arc_diedL[mons.indexDL], (sizeW, sizeH))
            else:
    ...
    def move(self, target):                         # Di chuyển theo nvc
        for mons in self.sprites():
            ...
            if mons.isAtt:  continue                  # Nếu trog trạng thái ra đòn
        ...
        if len(self) < 5:           # Giới hạn quái trên sân
            ...
            if rd >= 50 and rd <= 59:
                ...
                for mons in self.sprites():             # Check có trùng vt vs quái        
                    while mons.X == rd and mons.rect.collidepoint((rd, 222)) and self.target.rect.collidepoint((rd, 222)):
                        if len(self) % 2:
    ...
    def __init__(self, x, y, Hp, target):
        ...
        for i in range(4):
            ...
            if i > 0:
                str += chr(i + 48)
            str += '.png'
            self.arc_diedR.append(pygame.image.load(os.path.join(str)))

        for i in range(4):
            str = 'Images/SkeDeadMi'
            if i > 0:
                str += chr(i + 48)
            str += '.png'
            self.arc_diedL.append(pygame.image.load(os.path.join(str)))
