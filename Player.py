...
class Player(pygame.sprite.Sprite):
    ...
    def draw(self):
        ...
        # pygame.draw.rect(screen, (252, 0, 0), self.rect)
    ...
    def update(self):
        ...
        if self.dir > 0:
            self.rectAtt = pygame.Rect(self.X + self.rect.width - 40, self.Y , self.rect.width, self.rect.height)
            
        else:
            self.rectAtt = pygame.Rect(self.X - self.rect.width + 40, self.Y , self.rect.width, self.rect.height)
        self.rect = pygame.Rect(self.X, self.Y, sizeW, sizeH)
    ...
    def handle_event(self, target):
        ...
        if keys[pygame.K_j] and not self.jumping:
            if self.dir > 0:
                ...
                self.image = pygame.transform.scale(self.knight_attack_right[self.indexAR], (sizeW, sizeH))
                self.attack(target)
            else:
                self.indexAL = (self.indexAL + 1) % 4
                self.image = pygame.transform.scale(self.knight_attack_left[self.indexAL], (sizeW, sizeH))
                self.X -= 30
                self.isAtt = 1
                self.attack( target)
            ...
            self.image = pygame.transform.scale(self.knight_run_left[self.indexL], (sizeW, sizeH))
            ...
            self.image = self.image = pygame.transform.scale(self.knight_run_right[self.indexR], (sizeW, sizeH))
        ...
        if not self.jumping:
            ...
            self.image =  pygame.transform.scale(self.knight_jump[self.dirJump], (sizeW, sizeH))